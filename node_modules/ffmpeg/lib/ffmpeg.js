...
var ffmpeg = function (/* inputFilepath, settings, callback */) {
	...
	var _ffmpegInfoConfiguration = function (settings) {
		...
		let binaryPath = __dirname;
		if (process.platform === 'win32') {
			binaryPath = binaryPath.substring(0, binaryPath.lastIndexOf('\\'));
		} else {
			binaryPath = binaryPath.substring(0, binaryPath.lastIndexOf('/'));
		}
		if (process.platform === 'win32') {
			binaryPath = path.join(binaryPath, '\\binary\\win32\\ffmpeg.exe');
		} else if (process.platform === 'darwin') {
			binaryPath = path.join(binaryPath, '/binary/darwin/ffmpeg');
		} else if (process.platform === 'linux') {
			binaryPath = path.join(binaryPath, '/binary/linux/ffmpeg');
		} else {
			binaryPath = 'ffmpeg';
		}
		// Make the call to retrieve information about the ffmpeg
		utils.exec(['"' + binaryPath + '"', '-formats', '2>&1'], settings, function (error, stdout, stderr) {
	...
	var _videoInfo = function (fileInput, settings) {
		...
		// Make the call to retrieve information about the ffmpeg
		utils.exec(['"' + binaryPath + '"', '-i', fileInput, '2>&1'], settings, function (error, stdout, stderr) {
